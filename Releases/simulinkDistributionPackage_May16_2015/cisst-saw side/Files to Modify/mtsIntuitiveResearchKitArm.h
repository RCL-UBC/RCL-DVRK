...
protected:
    ...
    virtual void SetTorqueJointArm(const prmForceTorqueJointSet & newTorque);
    virtual void GetTorqueJointArm(vctDoubleVec & currentTorque) const;
    ...
    virtual void SetTrajectoryOnOrOff(const mtsBool & trajectoryRunning);
    ...
    bool logsEnabled; //must be manually set in init; not connected to UI
    int  logEntryIndex;

    //! Enable logs
    virtual void EnableLogs(const mtsBool & enable);

    //! Function to close logs or end entry
    virtual void CloseLogs(bool closingTime);

    //! Function to check if a file already exists, to not overwrite a file
    virtual void CheckFileExists(std::string * fname);
};
